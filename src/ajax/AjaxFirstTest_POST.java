...
public class AjaxFirstTest_POST extends HttpServlet {
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		/*
		response의 헤더값을 변경
		html을 요청하는 것은 정적리소스를 요청하는 작업이고 요청이 될때마다 
		서버에서 작업을 수행하고 실행해서 결과를 가져온다.
		이를 비효율적이라 판단하여, 기본설정이 동일한 요청이 들어왔을때 캐쉬에서 저장하고 있는 파일을 보여준다.
		결과를 캐쉬에 저장하면 다음 요청에 대한 결과가 안 나올 수 있기 때문에
		응답결과를 캐쉬에 저장하지 않고 서버에서 계속 재요청하도록 헤더값에 설정
		*/
